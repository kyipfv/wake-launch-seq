...
export default function PlanCard() {
  const [tomorrowPlan, setTomorrowPlan] = useState<Plan | null>(null);
  ...
  const loadPlans = async () => {
    ...
    try {
      ...
      const todaysPlan = {
        user_id: user.id,
        date: todayDate,
        sunrise_time: todaySunriseTime.toISOString(),
        advice: `${todayAdvice} - ${todayReason}`,
      ...
      const tomorrowCompletedKey = `plan_completed_${tomorrowDate}`;
      ...
      const tomorrowsPlan = {
        user_id: user.id,
        date: tomorrowDate,
        sunrise_time: tomorrowSunriseTime.toISOString(),
        advice: `${tomorrowAdvice} - ${tomorrowReason}`,
  ...
  const markAsCompleted = async () => {
    ...
    if (user?.id === 'demo-user') {
      const completedKey = `plan_completed_${todayPlan.date}`;
      localStorage.setItem(completedKey, JSON.stringify({
        completed: true,
        completedAt,
        activity: todayPlan.recommendation
      }));
      ...
      history.push({
        date: todayPlan.date,
        type: todayPlan.recommendation,
        completedAt,
        sunrise: todayPlan.sunrise_time
      });
  ...
  if (!todayPlan && !tomorrowPlan) return null;

  const renderPlanCard = (plan: Plan, title: string, isToday: boolean) => {
      ...
      <div style={{
        ...
        <div style={{marginBottom: '24px'}}>
          <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px'}}>
            <h3 style={{fontSize: '20px', fontWeight: '700', color: '#111827'}}>{title}</h3>
            {plan.completed && (
              <div style={{
                padding: '6px 12px',
        ...
        {/* Complete Button - Only show for today's plan */}
  ...
  return (
    <div>
      {/* Today's Plan */}
      {todayPlan && renderPlanCard(todayPlan, "Today's Launch Sequence", true)}
      
      {/* Tomorrow's Plan */}
      {tomorrowPlan && renderPlanCard(tomorrowPlan, "Tomorrow's Launch Sequence", false)}
    </div>
  );
}